                ...
                struct Estructura_ProductosEnAlmacen prod2)
{
    ...
    {
        if (strcmp(prod1[i].codigo, prod2.codigo) == 0)
            return;
    }
...
{
    out << left << setw(15) << c.dni << left << c.nombre << endl
        << left << c.categoria << endl;
    out << "No.  CODIGO" << right << setw(23) << "CANTIDAD" << right << setw(13) << "FECHA" << endl;
    for (int i = 0; i < c.numeroDePedidos; i++)
    {
        out << i + 1 << ")     " << setw(10) << left << c.pedidosRealizados[i].codigo << setw(15) << right << setprecision(2) << fixed
            << c.pedidosRealizados[i].cantidad << setw(10) << " ";
        imprimirFecha(out, c.pedidosRealizados[i].fecha);
        out << endl;
    }
    out << "Monto total comprado: " << setw(12) << right << setprecision(2) << fixed << c.montoTotalGastado << endl;
}

ofstream &operator<<(ofstream &out, struct Estructura_ProductosEnAlmacen *arr)
{
    out << "No ." << setw(12) << right << "Codigo" << setw(20) << right << "Descripcion" << setw(20) << right << "Precio unitario" << setw(12) << right << "Descuento" << endl;
    for (int i = 0; strcmp(arr[i].codigo, "X"); i++)
    {
        out << i << ")      " << setw(14) << left << arr[i].codigo << setw(40) << left
            << arr[i].descripcion << setw(10) << right << setprecision(2) << fixed << arr[i].precioUnitario << setw(14) << right << setprecision(2) << fixed
